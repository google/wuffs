...
RAC differs from most other archive formats, whether established like
[Tar](https://en.wikipedia.org/wiki/Tar_%28computing%29) and
[Zip](https://en.wikipedia.org/wiki/Zip_%28file_format%29) or newer like
[śiva](https://blog.sourced.tech/post/siva/). Even if those formats let you
seek inside an archive, seek points are named by strings (i.e. file names) and
are positioned only at source file boundaries. You cannot seek to a relative
offset (in what RAC calls `DSpace`) within a source file. They also often don't
support shared compression dictionaries embedded within an archive.

RAC differs from the [Riegeli](https://github.com/google/riegeli)
sequence-of-records format. A RAC file arguably contains what Riegeli calls
records (which are not files, as they don't have names) and both formats
support efficient seeking, but Riegeli seeks to a point in what RAC calls
...
format, even if the LevelDB Table string keys are re-purposed to encode
integer-valued `DSpace` seek points. A LevelDB Table, by itself, does not
support shared compression dictionaries.

A LevelDB Table index is also flat, unlike RAC's hierarchical index. In both
cases, a maliciously written index could contain multiple entries for the same
key, and different decoders could therefore produce different output for the
same source file - a potential security vulnerability. For LevelDB Tables,
verifying an index key's uniqueness requires scanning every key in the file,
which can be relatively expensive, and is therefore not done in practice. In
comparison, RAC's "Branch Node Validation" process (see below) ensures that
